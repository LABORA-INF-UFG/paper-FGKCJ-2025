...
def read_topology(file):
    ...
    slice_file = json.load(open("../topologies/topology_{}_vDUs_{}_nodes/slices.json".format(sys.argv[1], sys.argv[2]), 'r'))
    count = 0
...
def run_model(enumerable, zeta, experiment):
    ...
    myparams = CpoParameters(LogVerbosity="Terse", LogPeriod=500000, SearchType="IterativeDiving", TimeLimit=1*60)
    mdl = CpoModel()
    ...
    if msol:
        ...
        json.dump({"solution": {"cost": cost, "revenue": revenue, "profit": revenue-cost, "efficiency": msol.get_objective_value(), "time": time.time() - solving_time}}, open("new_plot_results/{}/{}.json".format(experiment, len(DUs)), 'w'))
    else:
        json.dump({"solution": {"cost": 1, "revenue": 0, "profit": 0, "efficiency": 0, "time": 0}}, open("new_plot_results/{}/{}.json".format(experiment, len(DUs)), 'w'))
    print("solution file in:", "new_plot_results/{}/{}.json".format(experiment, len(DUs)))
...
if __name__ == '__main__':
    ...
    read_topology("../topologies/topology_{}_vDUs_{}_nodes/topology_{}_vDUs_{}_nodes.json".format(q_vDUs, q_nodes, q_vDUs, q_nodes))
    read_paths("../topologies/topology_{}_vDUs_{}_nodes/paths.json".format(q_vDUs, q_nodes))
    DRCs = DRC_structure(experiment)
    print("Creation time:\t{}".format(time.time() - alocation_time_start))
    print("-----------------------------------------------------------------------------------------------------------")
    run_model(enumerable=[0]+[100 for _ in range(100)], zeta=0.55, experiment=experiment)
