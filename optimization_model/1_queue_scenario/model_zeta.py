...
class Slice():
    def __init__(self, id, vDU, max_delay, requirements, n_flows, UE_WAFC):
        self.id = id
        self.vDU = vDU
        self.max_delay = max_delay
        self.requirements = requirements
        self.UE_WAFC = UE_WAFC
...
def read_topology(file):
    ...
    slice_file = json.load(open("topologies/{}_vDUs_{}_nodes_{}/slices.json".format(sys.argv[1], sys.argv[2], sys.argv[3]), 'r'))
    count = 0
    for slice in slice_file["slices"]:
        if slice["type"] == "URLLC":
            if count + 1 not in slice_VDUs:
                slice_VDUs[count + 1] = []
            slices.append(Slice(count, slice["vDU"], slice["max_delay"], slice["requirements"], slice["n_flows"], slice["UE WAFC"]))
...
def DRC_structure():
    ...
    # id, cpu_CU, cpu_DU, ram_CU, ram_DU, Fs_CU, Fs_DU, delay_BH, delay_MH, bw_BH, bw_MH, q_CRs, proc_delay, URLLC_pkt, eMBB_pkt
    ...
    if experiment == "All_splits":
        DRCs = {1: DRC1, 2: DRC2, 4: DRC4, 6: DRC6, 8: DRC8, 9: DRC9}
    elif experiment == "Only_D-RAN":
        DRCs = {1: DRC1}
    elif experiment == "Only_C-RAN":
        DRCs = {9: DRC9}

    return DRCs


def run_model(enumerable, zeta, experiment):
    print("Running Model")
    print("-----------------------------------------------------------------------------------------------------------")
    U = 4
    G = 6
    ...
    myparams = CpoParameters(LogPeriod=500000, SearchType="IterativeDiving")#, TimeLimit=1*60)
    mdl = CpoModel()
    ...
    # take a random vDU ID
    vCU = 1
    vDU = DUs[0]["node"]
    # psi_R = mdl.sum(mdl.w[slice.id] * slice.UE_WAFC for slice in slices)
    gama = (U * (G + eta - 1))/(zeta * Fmax)
    gama = round(gama, 2)
    print("UE WAFC", gama)
    psi_R = mdl.sum(mdl.w[slice.id] * gama for slice in slices)
    ...
    for n in nodes:
        if n["ID"]:
            mdl.add(mdl.sum(mdl.z[slice.id, n["ID"]] for slice in slices) <= enumerable[n["ID"]]) # the amount of used resources must respect the node capacity
            # NODE CAPACITY MUST BE RESPECTED
            ...
            for slice in slices:
                mdl.add(mdl.z[(slice.id, n["ID"])] >= 0) # variable Z must be >= zero
                mdl.add(mdl.z[(slice.id, n["ID"])] <= enumerable[n["ID"]]) # variable Z must respect the enumerable set of options

    ####################### DEFINE CONSTRAINTS with variable X only ########################
    # FOR EACH IS USED A SINGLE PATH AND VNC
    for b in DUs:
        mdl.add(mdl.sum(mdl.x[it] for it in i if it[1] == b["ID"]) == 1)

    # EACH SLICE/VDU MUST USE A SINGLE PATH
    for slice in slices:
        mdl.add(mdl.sum(mdl.y[it] for it in y if it[1] == slice.id) == 1)
        # W_AUX VAR MUST BE 0 WHEN W IS 0 (NO USER ADMITTED)
        mdl.add(mdl.w_aux[slice.id] <= mdl.w[slice.id])
        # W_AUX VAR MUST BE 1 WHEN W > 0 (SOME USERS WAS ADMITTED)
        mdl.add(mdl.w_aux[slice.id] >= mdl.w[slice.id]/slice.n_flows)
        # W VAR MUST CANNOT ADMIT MORE FLOWS THEN N_FLOWS (INPUT)
        mdl.add(mdl.w[slice.id] <= slice.n_flows)
        mdl.add(mdl.w[slice.id] >= 0)
        # DELAY CONSTRAINT (1ms)
        # mdl.add(mdl.sum(mdl.sum(mdl.x[v, DU_node_to_ID[slice.vDU]] for v in DRCs if v in [1, 2, 4, 6, 8, 9]) * mdl.y[(p.id, slice.id)] * mdl.w_aux[slice.id] * (((eval(slice.requirements)[0]*(mdl.x_aux[DU_node_to_ID[slice.vDU]]/1000))/mdl.min(nodes[n]["R"] * mdl.z[slice.id, nodes[n]["ID"]]/enumerable[nodes[n]["ID"]] - (mdl.w[slice.id] - 1) * (eval(slice.requirements)[1]*(mdl.x_aux[DU_node_to_ID[slice.vDU]]/1000)) for n in p.seq if nodes[n]["ID"])) +
        ...
        mdl.add(mdl.sum(mdl.y[(p.id, slice.id)] * mdl.w_aux[slice.id] * (((eval(slice.requirements)[0]*(mdl.x_aux[DU_node_to_ID[slice.vDU]]/1000))/mdl.min(nodes[n]["R"] * mdl.z[slice.id, nodes[n]["ID"]]/enumerable[nodes[n]["ID"]] - (mdl.w[slice.id] - 1) * (eval(slice.requirements)[1]*(mdl.x_aux[DU_node_to_ID[slice.vDU]]/1000)) for n in p.seq if nodes[n]["ID"])) +
                        (mdl.sum((((12000*7175/1000)/(nodes[n]["R"]/10**6))/10**6) + ((((eval(slice.requirements)[0]*(mdl.x_aux[DU_node_to_ID[slice.vDU]]/1000)) + (eval(slice.requirements)[1] * (mdl.x_aux[DU_node_to_ID[slice.vDU]]/1000) *
                            mdl.sum((((12000*7175/1000)/(nodes[n2]["R"]/10**6))/10**6) for n2 in p.seq if nodes[n2]["ID"] and p.seq.index(n2) > p.seq.index(n)))) * (mdl.w[slice.id] - 1))/(nodes[n]["R"] * (mdl.z[slice.id, nodes[n]["ID"]]/enumerable[nodes[n]["ID"]]))) for n in p.seq if nodes[n]["ID"])) +
                        (len(p.seq) - 2) * (0.000005 * 5) + (mdl.w[slice.id] * eval(slice.requirements)[1])/10**9 * mdl.sum(mdl.x[it_x] * DRCs[it_x[0]].proc_delay for it_x in i if it_x[1] == DU_node_to_ID[slice.vDU]))
                        for p in paths if p.target == slice.vDU) <= slice.max_delay)

        # FLOWS DEMAND CONSTRAINT - flows must have its capacity requirement satisfied
        for n in nodes:
            if n["ID"]:
    ...
    # mdl.add(mdl.z[0, 15] == 0)
    # for slice in slices:
    #     for n in nodes:
    #         if n["ID"]:
    #             mdl.add(mdl.z[(slice.id, n["ID"])] <= mdl.sum(mdl.y[(p.id, slice.id)] for p in paths if n["ID"] in p.seq and p.target == slice.vDU) * enumerable[n["ID"]])
    ...
    if exists('/opt/ibm/ILOG/CPLEX_Studio201/cpoptimizer/bin/x86-64_linux/cpoptimizer'):
        ...
        # msol = mdl.solve(agent='local', execfile='/opt/ibm/ILOG/CPLEX_Studio221/cpoptimizer/bin/x86-64_linux/cpoptimizer')
        msol = mdl.solve(agent='local', execfile='/opt/ibm/ILOG/CPLEX_Studio2211/cpoptimizer/bin/x86-64_linux/cpoptimizer')
    ...
    if msol:
        ...
        for j in i:
            if msol[mdl.x[j]] > 0:
                ...
                cost += len(DRCs[j[0]].Fs_CU) * eta + len(DRCs[j[0]].Fs_DU)
                VNFs_CU += len(DRCs[j[0]].Fs_CU)
                VNFs_DU += len(DRCs[j[0]].Fs_DU)
                # vDU_node = DUs[j[2]]["node"]
                # if "vDU{}".format(vDU_node) not in solution_dict:
                #     solution_dict["vDU{}".format(vDU_node)] = {}
                # solution_dict["vDU{}".format(vDU_node)]["vRAN_path"] = paths[j[0]].seq
                # solution_dict["vDU{}".format(vDU_node)]["VNC"] = j[1]
        print("------------------------ Selected Paths URLLC ------------------------")
        for j in y:
            if msol[mdl.y[j]] > 0:
                print("Slice {} path: {}".format(j[1], paths[j[0]].seq))
                # solution_dict["vDU{}".format(slices[j[0]].vDU)]["slice_path".format(j[0])] = paths[j[0]].seq
        ...
        for j in z:
            if msol[mdl.z[j]] > 0:
                ...
                # if "nodes" not in solution_dict["vDU{}".format(slices[j[0]].vDU)].keys():
                #     solution_dict["vDU{}".format(slices[j[0]].vDU)]["nodes"] = {}
                # solution_dict["vDU{}".format(slices[j[0]].vDU)]["nodes"]["{}".format(j[1])] = msol[mdl.z[j]]/enumerable
        ...
        for j in w:
            if msol[mdl.w[j]] > 0:
                ...
                WAFC += msol[mdl.w[j]] * gama
                admitted_flows += msol[mdl.w[j]]
        for j in x_aux:
            print("{} pkt size to vDU {}".format(msol[mdl.x_aux[j]]/1000, j))
                # solution_dict["vDU{}".format(slices[j].vDU)]["admitted_flows".format(j)] = msol[mdl.w[j]]

        # json.dump({"solution": solution_dict}, open("topologies/topology_{}_vDUs_{}_nodes/solution.json".format(sys.argv[1], sys.argv[2]), 'w'))

        print("-------------------------------------------------------------------------------------------------------")
        print("Model solve time:\t{}".format(time.time() - solving_time))
        print("-------------------------------------------------------------------------------------------------------")

        if not exists("third_round_solutions/zeta_{}".format(zeta)):
            os.system("mkdir third_round_solutions/zeta_{}".format(zeta))
        json.dump({"solution": {"cost": cost, 
                                "WAFC": WAFC, 
                                "gama": gama,
                                "FO": WAFC-cost, 
                                "time": time.time() - solving_time,
                                "VNFs_CU": VNFs_CU,
                                "VNFs_DU": VNFs_DU,
                                "Admitted_Flows": admitted_flows}}, open("third_round_solutions/zeta_{}/{}.json".format(zeta, sys.argv[3]), 'w'), indent=4)
    else:
        json.dump({"solution": {"cost": 0, 
                                "WAFC": 0,
                                "gama": gama,
                                "FO": 0, 
                                "time": 0,
                                "VNFs_CU": 0,
                                "VNFs_DU": 0,
                                "Admitted_Flows": 0}}, open("third_round_solutions/zeta_{}/{}.json".format(zeta, sys.argv[3]), 'w'), indent=4)
...
if __name__ == '__main__':
    ...
    read_topology("topologies/{}_vDUs_{}_nodes_{}/topology_{}_vDUs_{}_nodes.json".format(q_vDUs, q_nodes, type_t, q_vDUs, q_nodes))
    read_paths("topologies/{}_vDUs_{}_nodes_{}/paths.json".format(q_vDUs, q_nodes, sys.argv[3]))
    DRCs = DRC_structure()
    print("Creation time:\t{}".format(time.time() - alocation_time_start))
    print("-----------------------------------------------------------------------------------------------------------")
    for zeta in [i/100 for i in range(5, 101, 5)]:
        print("Starting solve with zeta = {}".format(zeta))
        run_model(enumerable=[0, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20], zeta=zeta, experiment=experiment)
