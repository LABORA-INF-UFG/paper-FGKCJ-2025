...
def run_model(enumerable, zeta, experiment, eta):
    print("Running Model")
    print("-----------------------------------------------------------------------------------------------------------")
    U = 4
    G = 6
    ...
    # gama = round(gama, 2)
    gama = 0.118
    print("UE revenue", gama)
    psi_R = mdl.sum(mdl.w[slice.id] * gama for slice in slices)
    CU_cost = mdl.sum(mdl.x[(d, b["ID"])] * len(DRCs[d].Fs_CU) * eta for d in DRCs for b in DUs)
    ...
    if msol:
        ...
        if not exists("third_round_solutions/eta_{}".format(eta)):
            os.system("mkdir third_round_solutions/eta_{}".format(eta))
        json.dump({"solution": {"cost": cost, 
                                ...
                                "VNFs_CU": VNFs_CU,
                                "VNFs_DU": VNFs_DU,
                                "Admitted_Flows": admitted_flows}}, open("third_round_solutions/eta_{}/{}.json".format(eta, sys.argv[3]), 'w'), indent=4)
    else:
        json.dump({"solution": {"cost": 0, 
                                ...
                                "VNFs_CU": 0,
                                "VNFs_DU": 0,
                                "Admitted_Flows": 0}}, open("third_round_solutions/eta_{}/{}.json".format(eta, sys.argv[3]), 'w'), indent=4)
...
if __name__ == '__main__':
    ...
    print("-----------------------------------------------------------------------------------------------------------")
    for eta in [0.017, 0.2585, 0.5]:
        run_model(enumerable=[0, 20, 20, 20, 20, 20, 20, 20, 20, 20, 20], zeta=0, eta=eta, experiment=experiment)
